         ...
         if((last_close - g_nearest_res) >= 1 * g_pip)
           {
            ...
            double est_lot = CalculateLotSize(entry_price, g_setup_low);
            LogEvent("Setup Found", StringFormat("\"direction\":\"BUY\",\"price\":%f,\"lot_size\":%f", entry_price, est_lot));
           }
        }
     }
         ...
         if((g_nearest_sup - last_close) >= 1 * g_pip)
           {
            ...
            double est_lot = CalculateLotSize(entry_price, g_setup_high);
            LogEvent("Setup Found", StringFormat("\"direction\":\"SELL\",\"price\":%f,\"lot_size\":%f", entry_price, est_lot));
           }
        }
     }
  }
   ...
   if(g_setup_valid_long)
     {
      ...
      if(g_wick_formed_long && current_ask >= g_setup_high + 1 * g_pip)
        {
         ExecuteOrder(ORDER_TYPE_BUY, g_setup_high + 1 * g_pip, g_setup_low);
   ...
   if(g_setup_valid_short)
     {
      ...
      if(g_wick_formed_short && current_price <= g_setup_low - 1 * g_pip)
        {
         ExecuteOrder(ORDER_TYPE_SELL, g_setup_low - 1 * g_pip, g_setup_high);
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
     {
      ...
      if(!g_partial_closed && profit_pips >= 10.0)
        {
         MqlTradeRequest request;
         MqlTradeResult result;
         ZeroMemory(request);
      ...
      if(!g_trailing_activated && profit_pips >= 20.0)
        {
